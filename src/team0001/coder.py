...
GRAPHX = dict(
    zip(
        sorted(
            [
                x
                for x in ["".join(p) for p in product("ACGT", repeat=4)]
                if x[::-1].translate(trans) != x
                if "AA" not in x
                if "CC" not in x
                if "GG" not in x
                if "TT" not in x
...
def bytes_to_dna(data: bytes):
    if len(data) > MAX_BYTES - GRAPH_BYTES:
        return []
    results = []
    for x, graph in GRAPHX.items():
        result = x
        current_base = result[-1]
        for byte in data:
            for i in range(4):
                upper = byte >> (6 - 2 * i)
                current_base = index_base[graph[base_index[current_base]].index(upper)]
                result += current_base
                byte &= 0b111111 >> 2 * i
        results.append(result)
    return [result for result in results if 100 <= len(result) <= 200]
...
class Coder(DefaultCoder):
    ...
    def image_to_dna(self, input_image_path: str, need_logs: bool = True):
        ...
        struct.pack(">BB", MatrixType.COO.value << 4, RecordType.RECT.value << 2 | 0)
        dna_sequences = [
            max(
                bytes_to_dna(
                    struct.pack(
                ...
                key=calculate_compatible_score,
            )
            ...
            for seq in bytes_to_dna(
                self.error_correction.encode(
                    fill_bytes(
                        ...
                        MIN_BYTES - EC_BYTES - GRAPH_BYTES,
                    )
                )
            )
        ]
    ...
    def carbon_to_silicon(self, dna_sequence: str):
        if len(dna_sequence) % 4 != 0:
            return ValueError
        graph = GRAPHX.get(dna_sequence[:4])
        if graph is None:
            return ValueError
        count = 0
        ba = bytearray()
